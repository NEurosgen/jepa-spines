...
from data_utils.stohastic_graph import GraphAugment
class JEPAModel(pl.LightningModule):
    def __init__(self, cfg):
        ...
        # >>> добавлено <<<
        aug_cfg = getattr(cfg.model, "aug", {})
        self.ctx_as_target_prob = float(getattr(cfg.model, "ctx_as_target_prob", 0.0))
        self.lambda_ctx = float(getattr(cfg.model, "lambda_ctx", 0.0))
        self.augment = GraphAugment(
            edge_drop=float(aug_cfg.get("edge_drop", 0.1)),
            node_drop=float(aug_cfg.get("node_drop", 0.0)),
            feat_noise_std=float(aug_cfg.get("feat_noise_std", 0.05)),
            feat_mask_prob=float(aug_cfg.get("feat_mask_prob", 0.0)),
        )
    ...
    def forward(self, data):
        return self.student_encoder(data)
    ...
    def training_step(self, batch, batch_idx):
        ...
        device = self.device
        ctx_batch, tgt_batch, P = ctx_batch.to(device), tgt_batch.to(device), P.to(device)

        # student context
        z_ctx = self.student_encoder(ctx_batch)                  # [B, D]
        z_ctx_rep = torch.repeat_interleave(z_ctx, sizes.to(device), dim=0)  # [sum_T, D]
        hat_psi = self.predictor(z_ctx_rep, P)                   # [sum_T, 2]

        # --- teacher branch: либо реальный target, либо контекст с аугментациями ---
        use_ctx_target = torch.rand((), device=device) < self.ctx_as_target_prob

        with torch.no_grad():
            if use_ctx_target:
                ctx_aug = self.augment(ctx_batch)               # аугментированный контекст
                Z_y_ctx = self.teacher_encoder(ctx_aug)         # [B, D]
                Z_y = torch.repeat_interleave(Z_y_ctx, sizes, dim=0)
            else:
                Z_y = self.teacher_encoder(tgt_batch)           # [sum_T, D]

            psi = self.psi_from_target_latent(Z_y)              # [sum_T, 2]

        loss_main = F.smooth_l1_loss(hat_psi, psi, beta=self.smoothl1_beta, reduction="mean")
        lambda_cos = getattr(self.cfg.model, "lambda_cos", 0.1)
